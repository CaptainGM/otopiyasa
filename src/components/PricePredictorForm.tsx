...
import InteractiveDamageSelector from "@/components/InteractiveDamageSelector";
    ...
    <div className="space-y-4">
      ...
      <form onSubmit={submit} className="card grid gap-4 p-5 sm:grid-cols-2">
        ...
        <div className="sm:col-span-2">
          <InteractiveDamageSelector
            condition={condition}
            onChange={(c: Condition, note?: string) => {
              setCondition(c);
              if (note) {
                setPhotoNote((prev) => (prev ? `${prev} | ${note}` : note));
              }
            }}
          />
        </div>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {result && (
            <div className="card space-y-2 p-5">
              ...
              {(() => {
                const sampleCount = result.sampleSize;
                let confidenceScore = 60;
                let confidenceTitle = "Nadir Araç / Düşük Emsal";
                let confidenceText = "";
      ...
      {result && (
        <>
          {(() => {
            const advisory = brand && model ? findModelAdvisory(brand, model) : null;
          ...
          <ValuationReportModal
            car={{
              ...
              damageFlag: condition === "damaged",
              paintChange:
                condition === "painted"
                  ? "Boyalı / Değişenli Parçalar Mevcut"
                  : condition === "damaged"
