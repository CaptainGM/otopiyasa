...
Özet: MongoDB Atlas → veri migrasyonu → `vercel --prod` → env değişkenleri (`MONGODB_URI`, `JWT_SECRET`, `NEXT_PUBLIC_REQUIRE_LOGIN=1`, `SMTP*`) → Namecheap DNS. Scraper yerelde çalışıp aynı Atlas veritabanına yazar; canlı site o veriyi okur.
