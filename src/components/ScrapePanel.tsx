...
export function ScrapePanel({
  ...
  const todayStr = useMemo(() => initialToday?.date || getTurkeyDateStr(), [initialToday]);
  const yesterdayStr = useMemo(() => getTurkeyYesterdayStr(), []);
  ...
  const todayCount = useMemo(
    () => logs.filter((l) => getTurkeyDateStr(new Date(l.createdAt)) === todayStr).length,
    [logs, todayStr]
  );
  const yesterdayCount = useMemo(
    () => logs.filter((l) => getTurkeyDateStr(new Date(l.createdAt)) === yesterdayStr).length,
    [logs, yesterdayStr]
  );
  ...
  const filteredLogs = useMemo(() => {
    ...
    return logs.filter((log) => {
      const logDateStr = getTurkeyDateStr(new Date(log.createdAt));
  ...
  const effectiveToday = useMemo(() => {
    if (initialToday) return initialToday;
    let sc = 0;
    let ins = 0;
    let upd = 0;
    let dur = 0;
    let ops = 0;
    for (const l of logs) {
      const d = getTurkeyDateStr(new Date(l.createdAt));
      if (d === todayStr) {
        sc += l.scanned || 0;
        ins += l.inserted || 0;
        upd += l.updated || 0;
