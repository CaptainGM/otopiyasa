...
export default async function AdminPage() {
  ...
  try {
    const fetchPromise = Promise.all([
      Car.aggregate([
        { $match: { status: { $in: ["active", "removed"] } } },
        {
          $group: {
            _id: null,
            ...
            demoCount: {
              $sum: {
                $cond: [
                  { $and: [{ $eq: ["$status", "active"] }, { $eq: ["$sourceSite", "demo"] }] },
                  1,
                  0,
                ],
              },
      ...
      ManualScrapeLog.find()
        .select("actor source label scanned inserted updated deleted durationSeconds status message sampleVehicles createdAt")
        .sort({ createdAt: -1 })
        .limit(15)
        .lean()
        .catch(() => []),
  ...
  const isOnline =
    ...
    now.getTime() - lastHeartbeat.getTime() < 75 * 1000;

  const initialDaemon = {
    isOnline,
    host: heartbeat?.host || "Oracle Cloud Always Free (Frankfurt)",
    currentPhase: isOnline
      ? (heartbeat?.currentPhase || "Aktif Çalışıyor")
      : (heartbeat?.status === "stopped"
          ? "🛑 Durduruldu (run-daemon.bat ile başlatılabilir)"
          : "⚠️ Bağlantı Kesildi / Çevrimdışı (run-daemon.bat ile başlatılabilir)"),
    cycle: heartbeat?.cycle || 1,
    memoryMb: isOnline ? (heartbeat?.memoryMb || 45) : 0,
    uptimeSeconds: isOnline ? (heartbeat?.uptimeSeconds || 0) : 0,
    lastHeartbeat: heartbeat?.lastHeartbeat ? new Date(heartbeat.lastHeartbeat).toISOString() : null,
  ...
  const initialManualLogs = (manualScrapeDocs || []).map((m: any) => {
    const mDateStr = m.createdAt ? new Date(m.createdAt).toLocaleDateString("tr-TR") : "";
