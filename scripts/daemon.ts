...
async function main() {
  ...
  const { recordHourlyMetric, updateDaemonHeartbeat, getDaemonControl } = await import("@/models/ScrapeMetric");
  ...
  while (true) {
    ...
    if (ctrl.command === "stop" || ctrl.status === "stopped") {
      ...
      isIdle = true;
      currentPhase = "🛑 Durduruldu (run-daemon veya panelden başlatılabilir)";
      while (true) {
        await sleep(3000);
    ...
    if (currentMode === "new_only") {
      ...
      try {
        ...
        // 1. Çoklu Kaynak Süpürge: En eski 100 ilanı (Otomerkezi, Otokoç, DOD, Arabam...) canlı denetle
        const sweepLimit = currentMode === "sweep_only" ? 150 : 100;
        const sweepRes = await sweepAndCleanDeadListings({ limit: sweepLimit, concurrency: 3 });
        ...
        for (const [src, counts] of Object.entries(sourceMap)) {
          if (src === "arabam") continue;
          await recordHourlyMetric({
            source: src,
            scanned: counts.scanned,
            inserted: 0,
            updated: counts.active,
        ...
        const totalUpdated = arabamRefreshed + sweepRes.active;
        ...
        log(`✅ [ADIM 1 Tamamlandı] Güncellenen/Canlı: ${totalUpdated} | Silinen/Arşivlenen: ${totalDeleted}`);
        ...
        if (totalArabamScanned > 0 || arabamDeleted > 0) {
          ...
          await recordHourlyMetric({
            ...
            inserted: 0,
            updated: arabamRefreshed + arabamSweep.active,
    ...
    if (currentMode === "sweep_only") {
      ...
      // Her sitenin mevcut sayfa derinliğine göre tur bazlı dönüşümlü sayfalama yapılır.
      // Sürekli 1. sayfayı tekrar tekrar sorgulamak yerine katalogların derinliklerindeki
      // (sayfa 5, 10, 15, 20...) binlerce YENİ aracı çeker!
      const otokocPage = ((cycle - 1) * 3) % 65 + 1;
      const vavacarsPage = ((cycle - 1) % 3) + 1;
      const otoplusPage = ((cycle - 1) * 3) % 35 + 1;
      const otomerkeziPage = ((cycle - 1) * 2) % 18 + 1;
      const arabamPage = ((cycle - 1) * 2) % 30 + 1;
      ...
      const ALL_TARGETS: TargetItem[] = [
        ...
        { source: "carvak", label: "Carvak", query: "otomobil", limit: currentMode === "new_only" ? 35 : 25 },
        { source: "ikinciyeni", label: "İkinciyeni", query: "otomobil", limit: currentMode === "new_only" ? 35 : 25 },
        { source: "arabam", label: `Arabam (${arabamQuery.toUpperCase()} Sf.${arabamPage})`, query: arabamQuery, limit: currentMode === "new_only" ? 35 : 25, pageOffset: arabamPage },
      ];
      ...
      const results = await Promise.allSettled(
        activeTargets.map(async (target) => {
          try {
            ...
            log(`✅ [${target.label}] Yeni: +${res.inserted}, Güncellenen: ${res.updated}`);
            await recordHourlyMetric({
              source: target.source,
              scanned: res.sources?.[0]?.fetched || res.inserted + res.updated,
    ...
    // Nefes alma süresi
    const restSeconds = currentMode === "new_only" ? 10 : currentMode === "sweep_only" ? 10 : 15;
    isIdle = true;
    currentPhase = `☕ Dinleniyor (${restSeconds} sn mola, Sıradaki: Tur #${cycle + 1})`;
    await sendHeartbeat(currentPhase, "idle");
    log(`🎉 Tur #${cycle} bitti. ${restSeconds} saniye sonra bir sonraki tur başlayacak...\n`);
