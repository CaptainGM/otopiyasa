...
export async function scrapeIkinciyeniListings(
  ...
  let fetched = 0;
  reportProgress("İkinciyeni.com ihaleleri ve araçları sorgulanıyor", 1, 3);

  // 1. Adım: Aktif ve yaklaşan ihaleleri al
  let auctions: Array<{ id: string; label: string; totalCount: number }> = [];
  try {
    const res = await fetch(`${IKINCIYENI_API_URL}/GetAuctionsList`, {
      headers: {
        "User-Agent":
          "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0.0.0 Safari/537.36",
        Origin: IKINCIYENI_BASE_URL,
        Referer: `${IKINCIYENI_BASE_URL}/`,
      },
    ...
    console.warn("[İkinciyeni] GetAuctionsList hatası:", err instanceof Error ? err.message : err);
  }

  // 2. Adım: ListedVehicles üzerinden araçları sorgula
