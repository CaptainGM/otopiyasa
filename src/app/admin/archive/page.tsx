...
export default async function AdminArchivePage({ searchParams }: ArchivePageProps) {
  ...
  if (dateRange === "24h") {
    filter.updatedAt = { $gte: new Date(Date.now() - 24 * 60 * 60 * 1000) };
  } else if (dateRange === "3d") {
    filter.updatedAt = { $gte: new Date(Date.now() - 3 * 24 * 60 * 60 * 1000) };
    ...
    <div className="space-y-6 pb-12">
      ...
      <form method="GET" className="card p-4">
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-5">
          ...
          <div>
            <label className="text-xs font-semibold text-slate-400">Hasar Durumu</label>
            <select
              name="damage"
              defaultValue={damage}
            ...
            >
              ...
              <option value="painted">Boyalı / Lokal Boyalı</option>
              <option value="yes">Ağır Hasarlı Var</option>
              <option value="no">Hasarsız / Normal</option>
            </select>
          </div>
          <div>
            <label className="text-xs font-semibold text-slate-400">Kaldırılma Tarihi</label>
            <select
              name="dateRange"
        ...
        <div className="mt-3 flex items-center justify-end gap-2">
          {Boolean(q || brand || city || damage || dateRange) && (
            <Link
              href="/admin/archive"
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between border-t border-slate-800 pt-4 text-xs text-slate-400">
          ...
          <div className="flex items-center gap-2">
            {currentPage > 1 && (
              <Link
                href={`/admin/archive?page=${currentPage - 1}${q ? `&q=${encodeURIComponent(q)}` : ""}${brand ? `&brand=${encodeURIComponent(brand)}` : ""}${city ? `&city=${encodeURIComponent(city)}` : ""}${damage ? `&damage=${damage}` : ""}${dateRange ? `&dateRange=${dateRange}` : ""}`}
