...
describe("validateListingFields", () => {
  ...
  it("aşırı yüksek (trilyonluk) fiyatı reddeder", () => {
    expect(validateListingFields({ ...validListing, price: 999_999_999_999 }).valid).toBe(false);
  });
});
...
describe("checkPriceFloor", () => {
  ...
  it("aşırı yüksek fiyatı reddeder", () => {
    expect(checkPriceFloor(250_000_000, 1_000_000).ok).toBe(false);
  });
