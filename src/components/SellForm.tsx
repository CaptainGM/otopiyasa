...
export function SellForm({ initial, listingId, brands = [], brandModels = {} }: Props) {
  ...
  const [customModel, setCustomModel] = useState(
    initialModel && availableModels.length > 0 && !availableModels.includes(initialModel) ? initialModel : ""
  );
  const [isCustomModel, setIsCustomModel] = useState(
    initialModel ? (availableModels.length === 0 || !availableModels.includes(initialModel)) : false
  );
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const finalModel = isCustomModel
      ? customModel.trim()
      : (selectedModel === "__custom__" ? customModel.trim() : (selectedModel.trim() || String(fd.get("model") || "").trim()));

    if (!finalModel) {
      setError("Lütfen aracın modelini seçin veya yazın.");
      setLoading(false);
      return;
    }

    const payload = {
      brand: selectedBrand || fd.get("brand"),
      model: finalModel,
  ...
  return (
    <form onSubmit={handleSubmit} className="card space-y-5 p-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <div>
          ...
          <select
            ...
            onChange={(e) => {
              ...
              setSelectedModel("");
              setCustomModel("");
              const models = brandModels[newBrand] || [];
              setIsCustomModel(models.length === 0);
            }}
            required
          >
        ...
        {/* Model Seçimi / Özel Model Yazma */}
        <div>
          <div className="flex items-center justify-between mb-1">
            <label className="label mb-0" htmlFor="model">Model</label>
            {availableModels.length > 0 && (
              <button
                ...
                onClick={() => {
                  ...
                  if (next) setSelectedModel("__custom__");
                  else setSelectedModel(availableModels[0] || "");
                }}
          ...
          {isCustomModel || availableModels.length === 0 ? (
            <input
              ...
              placeholder="Örn. A3 Sportback, Taycan vb."
              required
            />
          ) : (
            <select
