...
export default async function FavoritesPage() {
  ...
  try {
    if (isUserWithFavorites(user)) {
      rawDocs = (user.favorites as unknown[]).filter(isLeanCarDoc);
  ...
  let recommendations: CarType[] = [];
  try {
    recommendations = await getRecommendationsForUser(authUser.userId, 6, rawDocs);
  } catch {
    recommendations = [];
  }
